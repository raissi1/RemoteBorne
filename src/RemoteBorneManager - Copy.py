...
class RemoteBorneApp:
    ...
    def _build_layout(self):
        ...
        self.active_entry = ttk.Entry(pq_frame)
        ...
        self.reactive_entry = ttk.Entry(pq_frame)
        ...
        self.btn_send_power = ttk.Button(
            pq_frame, text="Send", style="Accent.TButton", command=self.send_power_command
        )
        self.btn_send_power.grid(row=2, column=0, columnspan=2, pady=(4, 0), sticky="ew")
        ...
        self.cosphi_active_entry = ttk.Entry(cosphi_frame)
        ...
        self.cosphi_entry = ttk.Entry(cosphi_frame)
        ...
        # appliquer couleur au log selon thème
    ...
    def send_power_command(self):
        ...
        # Si le mode CosPhi est coché, on bloque (comme V7)
        ...
        # --- Parsing robuste avec valeur par défaut pour reactive = 0 ---
        try:
            active_val = int(float(active.strip()))
        except ValueError:
            self._popup_warning(
                "Warning", "Active must be a valid number."
            )
            return

        # Reactive : si vide ou invalide → 0 automatiquement
        try:
            raw_reactive = reactive.strip()
            reactive_val = int(float(raw_reactive)) if raw_reactive else 0
        except ValueError:
            reactive_val = 0

        # (optionnel) si tu veux refléter 0 dans le champ UI quand c'était invalide
        # self.reactive_entry.delete(0, "end")
        # self.reactive_entry.insert(0, str(reactive_val))
