# ==================================================================
# RemoteBorneManager_V5.py
# Layout "V2-like" (Remote Borne Control Interface, light theme)
# - Stable SSH via Plink (ssh_manager.py / plink_backend.py)
# - Same features as V2 + CosPhi mode exclusif
# ==================================================================
...
from energy_manager import EnergyManagerWindow
...
class RemoteBorneApp:
    def __init__(self, config: configparser.ConfigParser):
        ...
        self.local_path_default = paths_cfg.get("local_path", os.getcwd())
        ...
        # Heartbeat (echo alive) : compteur d'échecs consécutifs
        self._hb_failures = 0          # nombre d'échecs consécutifs
        self._hb_fail_threshold = 2    # après 2 échecs, on force la déconnexion
        ...
        self.energy_menu = None
        self.help_menu = None

        # Widgets refs (pour enable/disable)
        ...
        # CosPhi mode labels
        self.pq_mode_label = None
        self.cosphi_mode_label = None
        ...
        # Logos
        self.logo_left = None  # ex: renault
        self.logo_right = None  # ex: AVL
        self._load_logos()
    ...
    def _load_logos(self):
        ...
        - logo_left: ex. renault.png
        - logo_right: ex. avl.png
        """
        try:
            ...
            if os.path.exists(left_path):
                img = Image.open(left_path).resize((80, 30))
                self.logo_left = ImageTk.PhotoImage(img)

            if os.path.exists(right_path):
                img = Image.open(right_path).resize((80, 30))
    ...
    def _build_menu(self):
        ...
        self.file_menu.add_command(
            label="Copy to GridCodes", command=self.copy_selected_to_gridcodes
        )
    ...
    # ==================================================================
    # LAYOUT (comme ta V2, mais plus clean)
    # ==================================================================
    def _build_layout(self):
        ...
        self.pq_mode_label = ttk.Label(
            power_frame, text="Mode: P/Q", bootstyle=SUCCESS
        )
        self.pq_mode_label.pack(anchor="w", pady=(0, 3))
        ...
        self.cosphi_mode_label = ttk.Label(
            cos_frame, text="Mode: CosPhi", bootstyle=SECONDARY
        )
    ...
    def log(self, msg: str):
        ...
        # Pendant l'init, le thread SSH peut log avant la création du widget
    ...
    def _update_cosphi_ui(self):
        """
        Gestion du mode CosPhi / P-Q :
        - Si pas connecté : tout est grisé.
        - Si UseCosPhi = False → P/Q actif, CosPhi grisé.
        - Si UseCosPhi = True  → CosPhi actif, P/Q grisé.
        """
        ...
        if not self.connected:
            ...
            # Mode labels gris
            if self.pq_mode_label:
                self.pq_mode_label.configure(bootstyle=SECONDARY)
            if self.cosphi_mode_label:
                self.cosphi_mode_label.configure(bootstyle=SECONDARY)
            return
        ...
        if not use:
            # Mode P/Q
            if self.pq_mode_label:
                self.pq_mode_label.configure(bootstyle=SUCCESS)
            if self.cosphi_mode_label:
                self.cosphi_mode_label.configure(bootstyle=SECONDARY)
            ...
            if self.pq_mode_label:
                self.pq_mode_label.configure(bootstyle=SECONDARY)
            if self.cosphi_mode_label:
                self.cosphi_mode_label.configure(bootstyle=INFO)
    ...
    def _alive_monitor_thread(self):
        """Surveille la connexion en arrière-plan avec `echo alive`."""
        while not self._stop_alive:
            ...
            def cb(res):
                if res.get("success"):
                    if self._hb_failures > 0:
                        self.log("[ALIVE] Heartbeat OK à nouveau.")
                    self._hb_failures = 0
                    return

                self._hb_failures += 1
                self.log(
                    f"[ALIVE] Heartbeat failed (echo alive). "
                    f"Consécutifs = {self._hb_failures}"
                )

                if self._hb_failures >= self._hb_fail_threshold:
                    if not self.connected:
                        return

                    self.log("[ALIVE] Perte de connexion détectée. "
            ...
            try:
                ...
                self.log(f"[ALIVE ERROR] Exception in heartbeat: {e}")
    ...
    # ==================================================================
    # FILE OPERATIONS (copy/download/print/edit)
    # ==================================================================
    ...
    # ==================================================================
    # NETWORK / HELP / DEBUG / ENERGY
    # ==================================================================
    def open_energy_manager(self):
        """Ouvre la fenêtre Energy Manager PRO (plein écran, onglets P/Q, CosPhi, Historique, Monitor)."""
        if not self.connected or not self.ssh or not getattr(self.ssh, "connected", False):
            messagebox.showwarning(
                "Energy Manager",
                "Veuillez d'abord vous connecter à la borne avant d'ouvrir l'Energy Manager."
            )
            return
        try:
            EnergyManagerWindow(self.root, self.ssh)
            self.log("[ENERGY] Energy Manager PRO window opened.")
        except Exception as e:
            self.log(f"[ENERGY ERROR] Unable to open Energy Manager PRO: {e}")
            messagebox.showerror(
                "Energy Manager",
                f"Impossible d'ouvrir la fenêtre Energy Manager PRO :\n{e}"
            )
    ...
    def open_network_config(self):
        try:
            ...
            self.log(f"[ERROR] Import network_config failed: {e}")
            messagebox.showerror("Network", f"Import error:\n{e}")
            return

        # On mémorise les anciennes valeurs pour détecter les changements
        old_host = self.config.get("SSH", "host", fallback="")
        old_user = self.config.get("SSH", "username", fallback="")
        old_pwd  = self.config.get("SSH", "password", fallback="")
        old_port = self.config.get("SSH", "port", fallback="22")
        ...
        def on_saved():
            ...
            try:
                ...
                ssh_cfg = cfg["SSH"]
                new_host = ssh_cfg.get("host", "")
                new_user = ssh_cfg.get("username", "")
                new_pwd  = ssh_cfg.get("password", "")
                new_port = ssh_cfg.get("port", "22")
                ...
                if changed:
                    msg = (
                        "Network / SSH configuration has changed:\n"
                        f"- Host: {old_host} → {new_host}\n"
                        f"- User: {old_user} → {new_user}\n"
                        f"- Port: {old_port} → {new_port}\n\n"
                        "Restart application now to apply these changes?"
                    )
                    if messagebox.askyesno("Network configuration changed", msg):
                        ...
                        self.log("[NETWORK] User declined automatic restart.")
                else:
                    messagebox.showinfo(
                        "Network",
                        "Configuration not changed.\n"
                        "Current settings are already up to date.",
                    )
                    self.log("[NETWORK] No change detected in SSH settings.")
        ...
        try:
            network_config.open_network_config(self.root, on_saved)
        except TypeError:
            try:
                network_config.open_network_config(self.root)
                on_saved()
            except Exception as e:
                self.log(f"[ERROR] Unable to open network configuration: {e}")
    ...
    def force_reconnect(self):
        try:
            ...
            self.log("[SSH] Manual reconnect requested.")
        except Exception as e:
            self.log(f"[SSH ERROR] force_reconnect failed: {e}")
            messagebox.showerror("SSH", f"Reconnect failed:\n{e}")
    ...
    def show_about(self):
        messagebox.showinfo(
            ...
            "Windows version (Plink backend)\n"
            "GridCodes, Power/CosPhi commands, services restart,\n"
            "debug logs, network config.\n"
            "Signature: RNA.",
        )
