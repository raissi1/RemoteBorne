...
class RemoteBorneApp:
    def __init__(self, config: configparser.ConfigParser):
        ...
        self.root.title("Remote Borne Control Interface - V7")
        try:
    ...
    def _build_menu(self):
        ...
        file_menu.add_command(label="Connect", command=self.force_reconnect)
        file_menu.add_command(label="Disconnect", command=self._manual_disconnect)
        file_menu.add_separator()
        file_menu.add_command(label="Copy to GridCodes", command=self.copy_selected_to_gridcodes)
        file_menu.add_command(label="Download", command=self.download_selected)
        file_menu.add_command(label="Print", command=self.print_selected)
        file_menu.add_command(label="Edit", command=self.edit_selected)
        file_menu.add_separator()
        file_menu.add_command(label="Restart Services", command=self.restart_initd_services)
        file_menu.add_command(label="Reboot", command=self.reboot_device)
        file_menu.add_separator()
        ...
        tools_menu.add_command(label="Energy Manager PRO", command=self.open_energy_manager)
        tools_menu.add_command(label="Debug logs", command=self.open_debug_logs_window)
        tools_menu.add_command(label="Network config", command=self.open_network_config)
